...
The code written is tested on ten different types of testcase datasets which vary by many characteristics(some has more datapoints(millions), more number of feautures(>100), more number of outliers(hundreds of outliers) and **the code worked well on all these datasets with an NMI of more than 0.8 for all cases and ran in less than 150 seconds for all the cases**. 
